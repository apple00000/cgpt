...
class ChatGPTBot(Bot,OpenAIImage):
    def __init__(self):
        ...
        if config.conf().get('open_ai_api_base'):
            openai.api_base = config.conf().get('open_ai_api_base')
        proxy = config.conf().get('proxy')
        if proxy:
            openai.proxy = proxy
        if config.conf().get('rate_limit_chatgpt'):
            self.tb4chatgpt = TokenBucket(config.conf().get('rate_limit_chatgpt', 20))
    ...
    def reply(self, query, sys, context=None):
        ...
        if context.type == ContextType.TEXT:
            ...
            reply = None
            clear_memory_commands = config.conf().get('clear_memory_commands', ['#清除记忆'])
    ...
    def compose_args(self):
        return {
            "model": config.conf().get("model") or "gpt-3.5-turbo",  # 对话模型的名称
            "temperature":config.conf().get('temperature', 0.9),  # 值在[0,1]之间，越大表示回复越具有不确定性
            # "max_tokens":4096,  # 回复最大的字符数
            "top_p":1,
            "frequency_penalty":config.conf().get('frequency_penalty', 0.0),  # [-2,2]之间，该值越大则更倾向于产生不同的内容
            "presence_penalty":config.conf().get('presence_penalty', 0.0),  # [-2,2]之间，该值越大则更倾向于产生不同的内容
        }
